...
ComediScope::ComediScope( ComediRecord *comediRecordTmp, 
	...
	for (int dout =0; dout<2;dout++){
	  int ret = comedi_dio_config(dev[0],2,dout,COMEDI_OUTPUT);
	  if(ret < 0){
	    comedi_perror("dout config error");
	    exit(-1);
	  }
	}
...
ComediScope::~ComediScope() {
	//dout configuration
	for (int dout =0; dout<2;dout++){
	  int ret =  comedi_dio_write(dev[0],2,dout,0);
	  if(ret < 0){
	    comedi_perror("dout constructor error");
	    exit(-1);
	  }
...
void ComediScope::paintData(float** buffer) {
	...
	paint.drawLine(xer,0,
		       xer,h);
	int act=1;
	static float minV = -1.333;
	static float maxV = 1.333;
	for(int n=0;n<nComediDevices;n++) {
	  if (comediRecord->axis->hasChanged() != 0 ) {
	    int axis_mod = comediRecord->axis->getAxis();
	    switch (axis_mod){
	    case 0:
     	      minV = crange[n]->min;
	      maxV = crange[n]->max;
...
void ComediScope::paintEvent( QPaintEvent * ) {
	...
	while (1) {
	  ...
	  for(int n=0;n<nComediDevices;n++) {
	    int subdev_flags = comedi_get_subdevice_flags(dev[n], subdevice);
		...
		if ((n==fftdevno) && (ch==fftch) &&(comediRecord->fftscope))
	  ...
	  if (comediRecord->led->hasChanged() != 0 ) {
	    int led_mod = comediRecord->led->getLed();
	    fun++;
	    printf("led:%d fun:%d\n",led_mod,fun);
	    switch (led_mod){
	    case 0:
	      for (int dout =0; dout<2;dout++){
		ret =  comedi_dio_write(dev[0],2,dout,0);
		if(ret < 0){
		  comedi_perror("dout case  error");
		  exit(-1);
		}
	      }
	      break;
		...
		exit(-1);
	      ...
	      ret = comedi_dio_write(dev[0],2,0,1);
	      if(ret < 0){
		comedi_perror("dout case  error");
		exit(-1);
	      }
		...
		exit(-1);
	      ...
	      ret =  comedi_dio_write(dev[0],2,1,1);
	      if(ret < 0){
		comedi_perror("dout case  error");
		exit(-1);
	      }
