...
ComediRecord::ComediRecord( QWidget *parent, 
	...
	axisLabel->setText("Aaxis limit:");
	axisgrp = new QGroupBox;
	axisgrp->setStyleSheet(styleSheet);
	axisbox = new QHBoxLayout();
	axisgrp->setLayout(axisbox);
	axisbox->addWidget(axisLabel);
	axisbox->setSpacing(1);
	axis = new Axis();
	axis->setStyleSheet(styleSheet);
	axisbox->addWidget(axis);	
	restLayout->addWidget(axisgrp);

	//led stuff
	ledLabel = new QLabel(this);
	ledLabel->setText("LED mode:");
	ledgrp = new QGroupBox;
	ledgrp->setStyleSheet(styleSheet);
	ledbox = new QHBoxLayout();
	ledgrp->setLayout(ledbox);
	ledbox->addWidget(ledLabel);
	ledbox->setSpacing(1);
	led = new Led();
	led->setStyleSheet(styleSheet);
	ledbox->addWidget(led);	
	restLayout->addWidget(ledgrp);
